...
class HashMap:
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        # Confirm new_capacity is a prime number
        if not self._is_prime(new_capacity):
            new_capacity = self._next_prime(new_capacity)

        # Create updated table with new capacity
        new_table = HashMap(new_capacity, self._hash_function)

        # Rehash elements and put into new table
...
def find_mode(da: DynamicArray) -> tuple[DynamicArray, int]:
    ...
    mode_val = map.get_keys_and_values()
    mode_arr = DynamicArray()
    mode_freq = 0

    # Locate keys with max frequency
    for i in range(mode_val.length()):
        if mode_freq < mode_val.get_at_index(i)[1]:
            mode_freq = mode_val.get_at_index(i)[1]

    # Add mode keys to tuple
    for i in range(mode_val.length()):
